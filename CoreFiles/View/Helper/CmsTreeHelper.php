 ...
 * @link		 http://api.pyramidcms.com/docs/classCmsTreeHelper.html
 * @package		 Cms.Plugin.Helper 
 * @since		 Pyramid CMS v 1.0
 */
class CmsTreeHelper extends AppHelper {
 ...
 *	'ulClass' => a class which, along with the depth number, will be added to every UL in the list
 *	'liClass' => a class which, along with the depth number and list item number, will be added to every LI in the list
 *  'depthAdder' => optional integer to start the depth count at for appending to class names
 *  'extraData' => an array of extra data to send to the element
 *
	...
	public function generate($data, $settings = array()) {
		...
		foreach ($data as $i => &$result) {
			...
			if ($_addType) {
				...
				if ($type) {
					if ($ulClass != null) {
						$adder = $ulClass;
						if ($addDepth) {
							$adder .= ($depthAdder + $elementData['depth']);
						}
						$this->addTypeAttribute('class', $adder, null, 'previous');
					}
			...
			if ($itemType) {
				if ($liId) {
					$idToUse = null;
					if (isset($elementData['data'][$model]['id'])) {
						$idToUse = $elementData['data'][$model]['id'];
					} else if (isset($elementData['data']['id'])) {
						$idToUse = $elementData['data']['id'];
					}
					$this->addItemAttribute('id', $liId . $idToUse, null);
				}
				
				if ($liClass != null || $liClassOnFirst != null) {
					if ($liDepth != $elementData['depth']) {
						$liDepth = $elementData['depth'];
					}
					
					$liClasses = array();
					if (!isset($liCount[$liDepth])) {
						$liCount[$liDepth] = 1;
						if ($liClassOnFirst) {
							$liClasses[] = $liClassOnFirst;
						}
					}
					
					if ($liClass) {
						$liClasses[] = $liClass . $elementData['depth'] . '_' . $liCount[$liDepth];
					}
					
					$liClassString = null;
					if (!empty($liClasses)) {
						$liClassString = implode(' ', $liClasses);
					}
					
					$this->addItemAttribute('class', $liClassString , null);
