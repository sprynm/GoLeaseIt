...
class Media_Process_Adapter_Imagick extends Media_Process_Adapter {
	...
	public function __construct($handle, $background = "#ffffff00") {
		$this->_object = new Imagick();
		//$this->_object->setBackgroundColor( new ImagickPixel($background) );
		...
		//flatten the image so that the background colour applies
		//$this->_object = $this->_object->flattenImages();
		...
		$this->autoRotateImage($background);
	...
	public function convert($mimeType, $bgcolour = "#ffffff00") {
		...
		//since the image might have transparent pixels
		$this->_object->setImageBackgroundColor( $bgcolour );
		
		//flatten the image so that the background colour applies
		$this->_object = $this->_object->mergeImageLayers(Imagick::LAYERMETHOD_FLATTEN); //->flattenImages()
	...
	//since most browsers do not orient images based on exif data rotate the pixels in the image and reset the orientation exif data
	public function autoRotateImage($background = '#ffffff00') {

    $orientation = $this->_object->getImageOrientation();

    switch($orientation) {
        case imagick::ORIENTATION_BOTTOMRIGHT:
            $this->_object->rotateimage($background, 180); // rotate 180 degrees
        break;

        case imagick::ORIENTATION_RIGHTTOP:
            $this->_object->rotateimage($background,90); // rotate 90 degrees CW
        break;

        case imagick::ORIENTATION_LEFTBOTTOM:
            $this->_object->rotateimage($background, -90); // rotate 90 degrees CCW
        break;
    }

    // Now that it's auto-rotated, make sure the EXIF data is correct in case the EXIF gets saved with the image!
    return $this->_object->setImageOrientation(imagick::ORIENTATION_TOPLEFT);
	} 
}
