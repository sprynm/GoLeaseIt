...
class Media_Process_Image extends Media_Process_Generic {
	...
	public function __construct(array $config = array()) {
		$default = array('source' => null, 'adapter' => null, 'background'=>'#ffffff00');
		...
		if (is_object($adapter)) {
			...
			if ($adapter) {
				...
				$this->_adapter = new $class($source, $background);
			}
		}
	}
	 ...
	 * Adds a background colour to fill in to the top/bottom/left/right.
	 *
	 * @param integer $width
	 * @param integer $height
	 ...
	 * @return boolean
	 */
	public function fitInsideBG($width, $height, $bgcolour = '#ffffff', $gravity = 'center') {
	//
	...
	//
		$r		= $rx > $ry ? $rx : $ry;
	//
		$widthResize	= $this->_adapter->width() / $r;
	//
		$heightResize	= $this->_adapter->height() / $r;
	//
		list($widthResize, $heightResize) = $this->_normalizeDimensions($widthResize, $heightResize, 'maximum');
	//
		$this->_adapter->resize($widthResize, $heightResize);
	// Default to $x = 0 if for some reason the $width measurement isn't greater than 0.
		$x		= ($widthResize < $width) ? -1 * abs(($width - $widthResize) / 2): 0;
	// Default to $y = 0 if for some reason the $height measurement isn't greater than 0.
		$y		= ($heightResize < $height) ? -1 * abs(($height - $heightResize) / 2): 0;
	//
		return $this->_adapter->passthru('extentImage',array($width, $height, $x, $y));
	}
	 ...
	 * @return boolean
	 */
	public function fitInside($width, $height, $bgcolour = '#ffffff', $gravity = 'center') {
	 ...
	 * @return boolean
	 */
	public function fitOutside($width, $height, $bgcolour = '#ffffff', $gravity = 'center') {
	 ...
	 * @return boolean
	 */
	public function crop($width, $height, $bgcolour = '#ffffff', $gravity = 'center') {
	 ...
	 * @return boolean
	 */
	public function zoomFit($width, $height, $bgcolour = '#ffffff', $gravity = 'center') {
	 ...
	 * @return boolean
	 */
	public function zoomCrop($width, $height, $bgcolour = '#ffffff', $gravity = 'center') {
	 ...
	 * @return boolean
	 */
	public function fitCrop($width, $height, $bgcolour = '#ffffff', $gravity = 'center') {
	//
  
		$gravity	= (in_array($gravity, $this->gravity_values)) ? $gravity: 'center';
		$rx = $this->_adapter->width() / $width;
		$ry = $this->_adapter->height() / $height;
	 ...
	 */
	protected function _boxify($width, $height, $bgcolour = '#ffffff', $gravity = 'center') {
		switch ($gravity) {
			case 'center':
	 ...
	 */
	public function convert($mimeType, $bgcolour = "#ffffff00") {
		if (!$this->_adapter->convert($mimeType, $bgcolour)) {
			return false;
		}
