 ...
 * @subpackage media.models.behaviors
 */
class CmsCouplerBehavior extends ModelBehavior {
 ...
 */
	public function beforeSave(Model $Model) {
		...
		} else if (!empty($Model->data[$Model->alias]['basename']) && !empty($Model->data[$Model->alias]['id'])) {
			//if the basename changed and the file was previously saved then try to rename the versions for this file
			//remove invalid characters from the filename before saving
			$Model->data[$Model->alias]['basename'] = preg_replace( '/[^\w\-\.]/', "", $Model->data[$Model->alias]['basename']);
			$newName = $Model->data[$Model->alias]['basename'];
			$newShortName = substr( $newName, 0, strrpos( $newName, '.' ) );
			
			if (empty($newName) || empty($newShortName)){
				//you can't change the filename to be blank
				$Model->validationErrors['basename'] = 'You can\'t have an empty filename.';
				return false;
			}
			
			//logic for if the file name has been changed
			$oldAttachment = $Model->findById($Model->data[$Model->alias]['id']);
			
			if (!empty($oldAttachment) && $oldAttachment[$Model->alias]['basename'] != $newName) {
				//don't allow the file extension to be changed
				$oldExtension = array_pop( explode( ".", $oldAttachment[$Model->alias]['basename'] ) ) ;
				$newExtension = array_pop( explode( ".", $newName ) ) ;
				
				if ($newExtension != $oldExtension) {
					$Model->validationErrors['basename'] = 'You can\'t change the file extension from ' . $oldExtension . ' to ' . $newExtension;
					return false;
				}
				
				//make sure the name hasn't been taken by another attachment
				$matchingAttachment = $Model->find('first', array(
					'conditions' => array(
				...
				if (!empty($matchingAttachment)) {
					$Model->validationErrors['basename'] = 'The name ' . $newName . ' has already been taken.';
					return false;
				}
				
				//update the names for all versions of this file
				$return = $Model->renameVersions( $oldAttachment, $newName );
				
				//rollback the basename for the item
				return $return;
			}
		}
		return true;
 ...
 */
	public function beforeDelete(Model $Model, $cascade = true) {
		...
		$File = new File($file);
		//don't abandon the process if this file was already removed
		if ($File->exists()) {
			return $File->delete();
		} else {
			return true;
