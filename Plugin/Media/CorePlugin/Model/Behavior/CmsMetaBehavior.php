 ...
 * @subpackage media.models.behaviors
 */
class CmsMetaBehavior extends ModelBehavior {
 ...
 */
	public function setup(Model $Model, $settings = array()) {
		...
		$this->_cached[$Model->alias] = Cache::read('media_metadata_' . $Model->alias);
	}
 ...
 */
	public function __destruct() {
		foreach ($this->_cached as $alias => $data) {
			if ($data) {
 ...
 */
	public function metadata(Model $Model, $file, $level = 1) {
		...
		if (isset($this->_cached[$Model->alias][$checksum])) {
			$data = $this->_cached[$Model->alias][$checksum];
		}
		...
		$this->_cached[$Model->alias][$checksum] = $data;
		return $result;
	}
