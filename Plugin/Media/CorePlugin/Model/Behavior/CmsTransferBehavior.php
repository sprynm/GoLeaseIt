...
require_once '/usr/local/share/pyramidcms/V2/lib/Cms/Vendor/Tinify/Tinify/Exception.php';
require_once '/usr/local/share/pyramidcms/V2/lib/Cms/Vendor/Tinify/Tinify/ResultMeta.php';
require_once '/usr/local/share/pyramidcms/V2/lib/Cms/Vendor/Tinify/Tinify/Result.php';
require_once '/usr/local/share/pyramidcms/V2/lib/Cms/Vendor/Tinify/Tinify/Source.php';
require_once '/usr/local/share/pyramidcms/V2/lib/Cms/Vendor/Tinify/Tinify/Client.php';
require_once '/usr/local/share/pyramidcms/V2/lib/Cms/Vendor/Tinify/Tinify.php';
 ...
 * @subpackage media.models.behaviors
 */
class CmsTransferBehavior extends ModelBehavior {
 ...
 */
	public function beforeValidate(Model $Model) {
		...
		if (!$this->_prepare($Model, $file)) {
			$Model->invalidate('file', 'Error preparing file for saving. Developer: make sure the upload form has the type="file" atttribute.');
			return false;
		}
		return true;
	}
 ...
 */
	public function beforeSave(Model $Model) {
		...
		$this->runtime[$Model->alias]['isPrepared'] = false;
		...
		switch ($chain) {
			case 'file-upload-remote>>uploaded-file-local>>file-local':
				...
				if ($result && ($file['type'] == 'image/jpeg' || $file['type'] == 'image/png')) {
					//
					$this->_tinifyImage($destination['file']);
				}
				break;
 ...
 */
	protected function _alternativeFile($filename, $tries = 100) {
		//
		$return	= $filename;
		//
		$i	= 1;
		...
		if (file_exists($return)) {
			...
			while (file_exists($return)) {
				//
				$return	= $dirname . DS . $filename . '_' . $i . '.' . $extension;
				//
				$i++;
			}
		}
	...
	protected function _tinifyImage($image) {
		//
		\Tinify\setKey('qVfk5hdjh1HS9SypvLdSS0H5Sst6K5Yl');
		//
