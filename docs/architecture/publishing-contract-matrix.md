...
| Featured + success story panels | Prototype | Instance select: `home_stories_instance_id` or `home_stories_instance_slug`; section field: `home_stories_limit`; item fields: `name`, `kicker`, `description`, `cta_text`, `cta_link`, image | prototype ref + item fields + numeric | Required for section | `home_stories_limit` defaults to `2` if empty/invalid; rank order controls alternating layout | Prototype admin + Home page field for instance binding | `View/Layouts/home.ctp` -> `View/Elements/home/story_panels.ctp` | Active | Managed editorial story rows without hardcoded content |
...
| Footer CTA platter | Content Block + page field toggle | Visibility key `show_cta_platter` (default `on`), site default block `Site.article_cta_content_block_id`, optional page override `cta_platter_content_block_id`; legacy fallback uses `Site.article_cta_heading`, `Site.article_cta_body`, `Site.article_cta_link`, `Site.article_cta_text` | boolean + block ID (+ legacy text/link) | Optional | CTA shows by default when key is missing; accepts `1/0`, `true/false`, `yes/no`, `on/off`; block must be published | Page editor/custom fields + Settings + Content Blocks | `View/Elements/layout/footer.ctp` | Active | Page-level inclusion control with admin-managed rich CTA markup |
